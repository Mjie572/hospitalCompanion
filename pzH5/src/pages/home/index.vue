<template>
    <div class="header">
        ...
        <van-search v-model="searchValue" placeholder="找医院" shape="round" />
    </div>
    ...
    <van-row justify="space-around">
        <van-col v-for="(item, index) in homeData.nav2s" :key="item.id" class="center-img" @click="goOrderTwo(index)"
            span="11">
            <van-image :src="item.pic_image_url" />
        </van-col>
    </van-row>
    <van-row @click="goOrder(item)" class="yy-list" v-for="item in homeData.hospitals" :key="item.id"
...
//首页轮播滚动页面展示
//首页数据
const homeData = reactive({//reactive对象不能直接赋值，必须进行初始化后才可赋值
    hospitals: [],
    nav2s: [],
    navs: [],
    now: '',
